...
if (isNil "arInfoWorld_MKY") then {
	nul = [] execVM "\STAF_FRAMEWORK\scr\MKY\fnc\MKY_fnc_getInfoWorld.sqf";
...
if (isServer) then {
	if ((arInfoWorld_MKY select 0) == "sand") exitWith {nul = [] execVM "\STAF_FRAMEWORK\scr\MKY\sand\MKY_Sand_Server.sqf";};
};
...
if (hasInterface) then {
	0 = [] spawn {
		...
		if ((arInfoWorld_MKY select 0) == "sand") exitWith {
			...
			0 = MKY_arSandEFX execVM "\STAF_FRAMEWORK\scr\MKY\sand\MKY_Sand_Client.sqf";
		};
