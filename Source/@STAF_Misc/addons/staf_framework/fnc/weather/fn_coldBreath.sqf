...
_handle = [_condition] spawn {
	...
	while {(alive player) && {!(isNull player)} && {call _condition}} do {
		...
		//waitUntil {!(player getVariable ["STAF_SimpleSentence_Talking", false])};
		sleep 0.5;

		deleteVehicle _source;
		deleteVehicle _fog;

		_breathing = player getVariable ["STAF_Breathing", 15];
		_delay = time + (60 / _breathing);
		waitUntil {((time > _delay)/* OR (player getVariable ["STAF_SimpleSentence_Talking", false])*/)};
	};
};
