...
Purpose: Creates light snow fall around unit. Also checks if unit is inside a vehicle or building. (Effekts inspired by MYK)
----------
Parameters:
...
_handle = [_unit, _condition] spawn {
	...
	if (!(isNil "STAF_snowFallRunning") && {STAF_snowFallRunning}) exitWith {["Snow Falling (Blizzard): Already running!"] call BIS_fnc_error};
