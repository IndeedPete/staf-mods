...
_arsenalvar = _objectUnderCursor getVariable ["STAF_var_ww2japarsenal", false];
_actionID = _objectUnderCursor getVariable "STAF_var_ww2japarsenal_actionID";

if (_arsenalvar) then {
	[_objectUnderCursor, _actionID] remoteExec ["removeAction", 0, true];
	[_objectUnderCursor, true] call ace_arsenal_fnc_removeBox;
	_objectUnderCursor setVariable ["STAF_var_ww2japarsenal_actionID", nil, true];
	_objectUnderCursor setVariable ["STAF_var_ww2japarsenal", false, true];
	
	[objNull, "GERMAN ARSENAL REMOVED"] call BIS_fnc_showCuratorFeedbackMessage;
} else {
	[_objectUnderCursor] call STAF_WW2_fnc_ww2japarsenal;
